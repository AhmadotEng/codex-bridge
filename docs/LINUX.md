...
**Upgrading rc.1/rc.2 startup:** those candidates quoted `WorkingDirectory`, which systemd rejected as a non-absolute path. rc.3 corrects that directive. After preserving current work and upgrading the installation, rerun `autostart-enable --component daemon` to replace an inactive, Bridge-owned registration, then `start --background` and `status`. An active unit must first be stopped during a coordinated idle window. The repair retains the unit identity and saved configuration; it does not change SSH startup. Do not delete ownership metadata or edit an unrelated unit to bypass a registration conflict. Verify actual service startup separately from login/reboot recovery.

Fedora may add `/usr/lib/systemd/user/service.d/10-timeout-abort.conf` to every user service. rc.3 recognizes this exact vendor file only when its path is root-owned, contains no symlinks or group/other writable components, and its sole setting is `[Service]` / `TimeoutStopFailureMode=abort`. Additional or changed overrides still produce an ownership conflict and are preserved. Do not remove distribution policy or ownership checks to force startup.
