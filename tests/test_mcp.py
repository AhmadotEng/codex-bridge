...
class MCPProtocolTests(unittest.TestCase):
    ...
    def test_initialize_and_tools_are_available_without_daemon(self):
        ...
        self.assertEqual(len({tool["name"] for tool in listed}), 12)
    ...
    def test_readonly_annotations_do_not_mark_mutations_readonly(self):
        readonly = {"peer_status", "session_list", "session_get", "task_status", "task_wait"}
        for tool in TOOLS:
...
class LocalHTTPTests(unittest.TestCase):
    ...
    def test_missing_config_does_not_prevent_tool_discovery(self):
        ...
        self.assertEqual(len(server.handle(request("tools/list"))["result"]["tools"]), 12)
